...
export default {
  ...
  setup(props, {emit}) {
    ...
    let swiperStyle =  {}; // swiper样式
    const currentIndex = ref(1); // 当前的index
    ...
    onMounted(() => {
       ...
       * 滚动到正确的位置
       */
      const scrollContent = currentPosition => {
        // 0.设置正在滚动
        scrolling = true;
        // 1.开始滚动动画
      ...
      const touchEnd = e => {
        ...
        if (distance === 0) {
          return
        } else if (distance > 0 && currentMove > totalWidth * props.moveRatio) { // 右边移动超过0.5
          currentIndex.value--
        } else if (distance < 0 && currentMove > totalWidth * props.moveRatio) { // 向左移动超过0.5
          currentIndex.value++
        }
        // 3.移动到正确的位置
        scrollContent(-currentIndex.value * totalWidth);
